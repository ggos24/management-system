...
`vercel.json` declares the crons; each `path` is a serverless function in `api/*.ts` (`deadline-reminders`, `equipment-overdue`, `renewal-reminders`), all meant to run at `0 8 * * *` UTC (11:00 Europe/Kyiv in summer, 10:00 in winter — Vercel crons are UTC-only and do not follow DST). Vercel injects `Authorization: Bearer ${CRON_SECRET}`; the handlers use the service-role client and dedup through a claim table (`task_deadline_reminders`, `renewal_reminders`) or a `last_*_ping_at` column, so re-running one is idempotent. Trigger one by hand with `curl -H "Authorization: Bearer $CRON_SECRET" $PUBLIC_APP_ORIGIN/api/<name>`. **`api/*.ts` run as native Node ESM** (`package.json` has `"type": "module"`): a relative import from `lib/` must carry an explicit `.js` extension (`'../lib/renewals.js'`) — Vercel compiles and renames the `.ts` sources, and an extensionless specifier crashes the deployed function on load with a 500 before the auth check runs; Vite and Vitest resolve either form, so the mistake only shows in production. **The Vercel Hobby plan caps a project at two cron entries** and a third fails the deploy — chain an extra handler from an existing one instead (`api/renewal-reminders.ts` exports `runRenewalReminders()` for exactly that).
...
The app ships as an installable PWA via `vite-plugin-pwa` (configured in `vite.config.ts`). Manifest metadata (name, theme color, icons) lives in that plugin block — it is the single source of truth. A service worker is generated by Workbox at build time; `registerType: 'prompt'` means users see a sonner toast on update rather than an auto-reload (protects Tiptap/modal unsaved state). `components/PWAUpdater.tsx` drives the prompt; `components/OfflineBanner.tsx` shows a thin banner when `navigator.onLine` is false. Supabase calls intentionally bypass the SW (different origin) — the cached shell opens but data requires network.

Icons live in `public/` (`favicon.ico`, `pwa-*.png`, `maskable-icon-512x512.png`, `apple-touch-icon-180x180.png`) and are generated from `public/logo.svg` via `npm run generate-pwa-assets` (config in `pwa-assets.config.ts`, preset `minimal-2023`). Regenerate and commit when the logo changes. `devOptions.enabled` is `false` so the SW stays out of `npm run dev`; test PWA behavior via `npm run build && npm run preview`.
