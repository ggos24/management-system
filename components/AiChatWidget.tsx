...
export const AiChatWidget: React.FC = () => {
  ...
  return (
    <>
      {/* FAB toggle — always visible when chat is closed */}
      {!isAiChatOpen && (
        <button
          onClick={() => setIsAiChatOpen(true)}
          className="fixed bottom-6 right-6 z-40 p-3.5 bg-black dark:bg-white text-white dark:text-black rounded-full shadow-lg hover:opacity-90 transition-opacity"
        >
          <Wand2 size={20} />
        </button>
      )}
      <div
        className={`fixed z-40 flex flex-col items-end transition-all duration-300 ${isAiChatOpen ? 'inset-0 md:inset-auto md:bottom-6 md:right-6 md:w-[400px]' : 'bottom-6 right-6 w-0 overflow-hidden'}`}
      >
        {isAiChatOpen && (
          <div className="bg-white dark:bg-zinc-900 border-0 md:border border-zinc-200 dark:border-zinc-800 md:rounded-lg shadow-2xl md:mb-4 w-full h-full md:h-[500px] flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 fade-in duration-200">
