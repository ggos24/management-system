...
const EquipmentTestSheet = React.lazy(() =>
  import('./EquipmentTestSheet').then((m) => ({ default: m.EquipmentTestSheet })),
);
...
const EquipmentLabels: React.FC = () => {
  ...
  // Calibration is a different job from production printing: one specimen, many
  // renderings, nothing to mark as printed afterwards.
  const [mode, setMode] = useState<'labels' | 'test'>('labels');
  const [specimenId, setSpecimenId] = useState<string>('');
  const [testSizes, setTestSizes] = useState(true);
  const [testEc, setTestEc] = useState(true);
  const [testLayouts, setTestLayouts] = useState(true);

  const sheet = SHEET_PRESETS.find((p) => p.value === preset) ?? SHEET_PRESETS[0];
  // One real code, so the printed samples resolve to a real unit when scanned.
  const specimen = equipmentItems.find((item) => item.id === specimenId) ?? equipmentItems[0];
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              variant="ghost"
              size="sm"
              disabled={mode === 'labels' ? !ready : !specimen}
              onClick={() => window.print()}
            >
              <Printer size={14} className="mr-1.5" />
              Print {mode === 'labels' && chosen.length > 0 ? chosen.length : ''}
            </Button>
            {/* Nothing to mark on a calibration sheet — it never goes on a shelf. */}
            {mode === 'labels' && (
              <Button
                size="sm"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-6 px-4 md:px-6 py-5">
            <div className="space-y-4 print:hidden">
              <p className="text-xs text-zinc-500 leading-relaxed">
                One code, printed every way worth trying, on a single A4. Print it, cut it up, stick the samples on real
                gear and scan them — the wall between “works” and “doesn’t” is a physical fact, not a setting.
              </p>

              <div>
                <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-1.5">Specimen</p>
                <CustomSelect
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full"
                  aria-label="Sample size in millimetres"
                />
              </div>

              <div className="space-y-1.5">
                <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Include</p>
                {(
                  [
                    { checked: testSizes, set: setTestSizes, label: 'Size sweep (14–32 mm)' },
                    { checked: testEc, set: setTestEc, label: 'Error-correction comparison' },
                    { checked: testLayouts, set: setTestLayouts, label: 'Layout variants' },
                  ] as const
                ).map((row) => (
                  <label key={row.label} className="flex items-center gap-2 text-xs text-zinc-600 dark:text-zinc-300">
            ...
            <div>
              {specimen ? (
                <React.Suspense fallback={<p className="text-sm text-zinc-400 print:hidden">Loading…</p>}>
                  <EquipmentTestSheet
                    item={specimen}
                    includeSizes={testSizes}
                    includeEc={testEc}
                    includeLayouts={testLayouts}
                    ecForSweep={ec as 'M' | 'Q' | 'H'}
                    layoutSizeMm={qrMm}
                  />
                </React.Suspense>
              ) : (
                <p className="text-sm text-zinc-400 print:hidden">
                  Register a unit first — the sheet prints a real code.
                </p>
              )}
            </div>
          </div>
            ...
            <div className="space-y-5 print:hidden">
              ...
              <div className="space-y-3">
                ...
                <p className="text-[11px] text-zinc-400 leading-relaxed">
                  Print one sheet first. Stick it on real gear and scan from 30 cm and 10 cm, in poor light, at an
                  angle, on the oldest phone in the team. A wasted sheet beats a wasted day.
                </p>
              </div>
            </div>
