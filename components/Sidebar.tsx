...
import { LayoutDashboard, FileText, Video, Instagram, Calendar, Settings, LogOut, Plus, Briefcase, ChevronLeft, ChevronRight, Hash, Globe, Cpu, MoreHorizontal, Megaphone, Monitor, PenTool, Code, Smartphone, Zap, Database, Layout, GripVertical, CheckCircle2 } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    ...
    <aside className={`fixed md:static inset-y-0 left-0 z-40 bg-zinc-50 dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 flex flex-col transition-all duration-300 ${sidebarWidth} ${mobileClass} md:translate-x-0`}>
      <div className={`p-4 flex items-center ${isCollapsed ? 'justify-center flex-col gap-4' : 'justify-between'} h-16`}>
        ...
        {!isCollapsed && (
            <div className="flex items-center gap-3 overflow-hidden animate-in fade-in duration-200">
                <img src="/logo.svg" alt="Logo" className="w-6 h-6 object-contain rounded-sm" onError={(e) => {
      ...
      <div className="flex-1 overflow-y-auto px-2 space-y-6 custom-scrollbar mt-6">
          ...
          <div className="space-y-0.5">
             <button
              ...
              >
                ...
                <span className={`truncate transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 hidden' : 'w-auto opacity-100'}`}>My Workspace</span>
              </button>
             <button
              ...
              >
                ...
                <span className={`truncate transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 hidden' : 'w-auto opacity-100'}`}>Dashboard</span>
              </button>
              <button
              ...
              >
                <Calendar size={18} />
                 <span className={`truncate transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 hidden' : 'w-auto opacity-100'}`}>Schedule</span>
              </button>
          </div>
          ...
          <div>
            <div className={`px-3 flex items-center justify-between mb-2 ${isCollapsed ? 'justify-center' : ''}`}>
                 {!isCollapsed && <h3 className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Workspaces</h3>}
                 {isCollapsed && <div className="h-px w-4 bg-zinc-200 dark:bg-zinc-800"></div>}
                 
                 {!isCollapsed && userRole === 'admin' && (
                     <button onClick={onManageTeams} className="text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200" title="Manage Teams">
                         <Settings size={12} />
                     </button>
                 )}
            </div>
            ...
            <nav className="space-y-0.5">
                {teams.filter(t => !t.hidden && !t.archived).map((team) => {
                    ...
                    <button
                        ...
                        title={team.name}
                    >
                        <Icon size={18} className={`${isActive ? 'text-black dark:text-white' : 'text-zinc-400'} flex-shrink-0`} />
                        <span className={`truncate transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 hidden' : 'w-auto opacity-100'}`}>{team.name}</span>
      ...
      <div className="mt-auto p-2 border-t border-zinc-200 dark:border-zinc-800 space-y-1">
        ...
        >
          <Settings size={18} />
           <span className={`truncate transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 hidden' : 'w-auto opacity-100'}`}>Settings</span>
        </button>
         <button 
            onClick={onLogout}
        ...
        >
          <LogOut size={18} />
           <span className={`truncate transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 hidden' : 'w-auto opacity-100'}`}>Logout</span>
        </button>
      </div>
    </aside>
    </>
