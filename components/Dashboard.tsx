...
import { ChevronDown, Calendar as CalendarIcon, X, ArrowRight, ChevronLeft, ChevronRight, Briefcase } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ tasks, members, absences, teams }) => {
  ...
  useEffect(() => {
      ...
      if (visibleTeams.length > 0 && (!teamFilter || teamFilter === 'management' || teams.find(t => t.id === teamFilter)?.hidden)) {
  ...
  const filteredTasks = useMemo(() => {
      return tasks.filter(task => {
          if (task.type !== teamFilter) return false;
  ...
  const cardClass = "bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white p-6 rounded-lg shadow-sm flex flex-col items-center justify-center text-center h-32 border border-zinc-200 dark:border-zinc-800";
    ...
    <div className="p-8 space-y-8 animate-fade-in bg-white dark:bg-black min-h-full font-sans">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            ...
            <p className="text-zinc-500 text-sm mt-1">Overview for <span className="font-semibold text-zinc-900 dark:text-zinc-300">{teams.find(t => t.id === teamFilter)?.name}</span></p>
        </div>
        <div className="flex flex-wrap gap-3 items-center">
             <div className="flex items-center gap-1 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-md px-3 py-1.5 shadow-sm hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
                ...
                />
                {(startDate || endDate) && (
                    <button onClick={() => { setStartDate(''); setEndDate(''); }} className="ml-2 p-0.5 rounded-full hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-400 hover:text-red-500"><X size={10} /></button>
                )}
             </div>
            <div className="w-[160px]">
                <CustomSelect 
                    icon={Briefcase}
                    options={teams.filter(t => t.id !== 'management' && !t.hidden).map(t => ({value: t.id, label: t.name}))}
                    value={teamFilter}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="flex flex-1 overflow-hidden">
                  <div className="w-1/2 pr-4 overflow-y-auto custom-scrollbar space-y-3">
                      {pieData.map((entry, index) => (
                          <div key={index} className="flex items-center justify-between p-2 rounded hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                              <div className="flex items-center gap-3">
                                  <div className="w-3 h-3 rounded-full shadow-sm border border-zinc-200 dark:border-zinc-700" style={{ backgroundColor: getColorForStatusIndex(index) }}></div>
                  ...
                  <div className="w-1/2 h-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            ...
                            <Tooltip 
                                contentStyle={{ backgroundColor: '#fff', borderRadius: '4px', border: '1px solid #e4e4e7', color: '#000' }}
          ...
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-lg border border-zinc-200 dark:border-zinc-800 h-[400px] flex flex-col">
              <div className="flex justify-between items-center mb-6">
                  <h3 className="text-lg font-bold text-zinc-900 dark:text-white">Team Workload <span className="text-xs font-normal text-zinc-500 ml-1">(Primary Owner)</span></h3>
              </div>
              <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4 pr-2">
                  {workloadData.map(({ member, total, statusCounts, overdueCount }) => (
                      <div key={member.id} className="flex items-start gap-4 p-3 rounded-lg border border-zinc-100 dark:border-zinc-800/50 hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
                          <img src={member.avatar} alt={member.name} className="w-10 h-10 rounded-full object-cover border border-zinc-200 dark:border-zinc-700 grayscale" />
                          <div className="flex-1 min-w-0">
                              <div className="flex justify-between items-center mb-2">
                                  <h4 className="text-sm font-bold text-zinc-900 dark:text-white truncate">{member.name}</h4>
                                  <div className="flex gap-2">
                                    {overdueCount > 0 && (
                                        <span className="text-xs font-bold bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300 px-2 py-0.5 rounded-full">{overdueCount} Overdue</span>
                                    )}
                                    <span className="text-xs font-bold bg-zinc-100 text-zinc-900 dark:bg-zinc-800 dark:text-white px-2 py-0.5 rounded-full">{total} Tasks</span>
                                  </div>
                              </div>
                              <div className="flex flex-wrap gap-2">
                                  {Object.entries(statusCounts).map(([status, count], idx) => (
                                      <div key={status} className="flex items-center gap-1.5 bg-zinc-50 dark:bg-zinc-900 px-2 py-1 rounded text-[10px] font-medium text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-800">
                                          <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: getColorForStatusIndex(idx) }}></div>
                  ...
                  {workloadData.length === 0 && <div className="text-center text-zinc-500 py-10">No team members assigned tasks.</div>}
              </div>
          </div>
      </div>
          ...
          <div className="flex-1 w-full min-h-0">
              <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={progressData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                      ...
                      <Tooltip cursor={{ fill: 'transparent' }} contentStyle={{ backgroundColor: '#fff', borderRadius: '4px', border: '1px solid #e4e4e7', color: '#000' }} />
                      <Legend iconType="circle" iconSize={8} wrapperStyle={{ paddingTop: '20px', fontSize: '12px', fontWeight: 'bold' }} />
                      {progressData.length > 0 && Object.keys(progressData[0]).filter(key => key !== 'name').map((key, index) => (
                              <Bar key={key} dataKey={key} stackId="a" fill={getColorForStatusIndex(index)} radius={[0, 0, 0, 0]} barSize={40} />
                      ))}
                  </BarChart>
              </ResponsiveContainer>
          </div>
