...
import { STATUS_COLORS, PRIORITY_COLORS, TEAM_STATUSES } from '../constants';
import { Plus, MoreHorizontal, Calendar as CalendarIcon, User, Wand2, LayoutGrid, List, Filter, ChevronLeft, ChevronRight, Hash, X, ChevronDown, CheckCircle, Settings2, Edit2, Trash2, GripVertical, FileText, Paperclip, ChevronUp, Link as LinkIcon, MapPin, GripHorizontal, Copy, Archive, FolderArchive, Type, List as ListIcon, Users } from 'lucide-react';
...
const Workspace: React.FC<WorkspaceProps> = ({ 
    tasks, teamFilter, teamName, members, currentUserId, 
    onUpdateTaskStatus, onAddTask, searchQuery, onTaskClick, 
    onOpenAiChat, onUpdateTask, teamStatuses, onUpdateTeamStatuses,
    archivedStatuses, onArchiveStatus, onDuplicateStatus,
    customProperties = [], onAddProperty, onUpdateProperty, onDeleteProperty, userRole
}) => {
  ...
  const currentStatusList = useMemo(() => {
      if (teamFilter === 'my-work') {
          ...
          const myTasks = tasks.filter(t => t.assigneeIds.includes(currentUserId) || t.contentInfo?.editorIds?.includes(currentUserId) || t.contentInfo?.designerIds?.includes(currentUserId));
  ...
  const [allPlacements, setAllPlacements] = useState<string[]>(['YouTube Shorts', 'Instagram Reels', 'Twitter Post', 'Main Page', 'Newsletter', 'Stories', 'Ticker', 'Push']);
  ...
  const filteredTasks = useMemo(() => {
    return tasks.filter(t => {
        ...
        const matchPlacement = filterPlacements.length === 0 || t.placements.some(tag => filterPlacements.includes(tag));
  ...
  const handleDeleteColumn = (id: string) => {
      if(userRole !== 'admin') {
          alert('Only admins can delete statuses.');
          return;
      }
        ...
        <div className="flex items-center justify-between">
             <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-1">
                    <button onClick={() => setViewMode('table')} className={`flex items-center gap-1.5 px-2 py-1 rounded transition-all text-xs font-medium ${viewMode === 'table' ? 'bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white' : 'text-zinc-500 hover:text-zinc-900 hover:bg-zinc-50 dark:hover:bg-zinc-900'}`}>
                        <List size={14} /> Table
                    </button>
                    <button onClick={() => setViewMode('board')} className={`flex items-center gap-1.5 px-2 py-1 rounded transition-all text-xs font-medium ${viewMode === 'board' ? 'bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white' : 'text-zinc-500 hover:text-zinc-900 hover:bg-zinc-50 dark:hover:bg-zinc-900'}`}>
                        <LayoutGrid size={14} /> Board
                    </button>
                    <button onClick={() => setViewMode('calendar')} className={`flex items-center gap-1.5 px-2 py-1 rounded transition-all text-xs font-medium ${viewMode === 'calendar' ? 'bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white' : 'text-zinc-500 hover:text-zinc-900 hover:bg-zinc-50 dark:hover:bg-zinc-900'}`}>
        ...
        {viewMode === 'board' && displayColumns.length > 0 && (
            <div className="flex gap-6 overflow-x-auto pb-4 h-full">
                {displayColumns.map(col => {
                    ...
                    >
                        <div className={`flex items-center justify-between px-1 mb-3 pb-2 group/header ${'cursor-grab active:cursor-grabbing'}`}>
                             {isCollapsed ? (
                                <div className="flex flex-col items-center gap-4 h-full py-4 cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded" onClick={() => toggleSection(col.id)}>
                                    <span className="text-[10px] bg-zinc-100 dark:bg-zinc-800 text-zinc-500 font-bold px-1.5 rounded">{filteredTasks.filter(t => t.status === col.id).length}</span>
                                    <div className="writing-mode-vertical text-xs font-bold text-zinc-500 tracking-wider whitespace-nowrap rotate-180" style={{ writingMode: 'vertical-rl' }}>{col.label}</div>
                                ...
                                <div className="flex-1">
                                    {editingColumnId === col.id ? (
                                        ...
                                        <div className="flex items-center gap-2 cursor-pointer">
                                            ...
                                            <GripHorizontal className="text-zinc-300 hover:text-zinc-500 flex-shrink-0 cursor-grab active:cursor-grabbing" size={14} />
                                            <button onClick={(e) => { e.stopPropagation(); toggleSection(col.id); }} className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300"><ChevronDown size={14}/></button>
                                            <span className="bg-zinc-100 dark:bg-zinc-800 text-zinc-500 text-[10px] px-1.5 py-0.5 rounded font-medium">{filteredTasks.filter(t => t.status === col.id).length}</span>
                                            <span 
                                ...
                                {teamFilter !== 'my-work' && (
                                    <div className="relative">
                                        <div className="opacity-0 group-hover/header:opacity-100 flex items-center transition-opacity">
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); setActiveColumnMenu(activeColumnMenu === col.id ? null : col.id); }}
                                                className="text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 p-1"
                                            >
                                                <MoreHorizontal size={14} />
                                            </button>
                                            <button onClick={() => onAddTask({ status: col.id })} className="text-zinc-400 hover:text-black dark:hover:text-white transition-colors p-1"><Plus size={14} /></button>
                                        </div>
                                        
                                        {activeColumnMenu === col.id && (
                                            <div className="absolute right-0 top-6 w-48 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg shadow-xl z-50 py-1 flex flex-col text-xs" onClick={(e) => e.stopPropagation()}>
                                                <button onClick={() => handleStartRename(col.id, col.label)} className="text-left px-3 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center gap-2">
                                                    <Edit2 size={12} /> Rename
                                                </button>
                                                <button onClick={() => { onDuplicateStatus(teamFilter, col.id, false); setActiveColumnMenu(null); }} className="text-left px-3 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center gap-2">
                                                    <Copy size={12} /> Duplicate (Empty)
                                                </button>
                                                <button onClick={() => { onDuplicateStatus(teamFilter, col.id, true); setActiveColumnMenu(null); }} className="text-left px-3 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center gap-2">
                                                    <Copy size={12} /> Duplicate (With Data)
                                                </button>
                                                <div className="h-px bg-zinc-100 dark:bg-zinc-800 my-1"></div>
                                                <button onClick={() => { onArchiveStatus(teamFilter, col.id); setActiveColumnMenu(null); }} className="text-left px-3 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center gap-2 text-yellow-600 dark:text-yellow-400">
                                                    <Archive size={12} /> {isArchived ? 'Unarchive' : 'Archive'}
                                                </button>
                                                <button onClick={() => handleDeleteColumn(col.id)} className="text-left px-3 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center gap-2 text-red-600">
                        ...
                        {!isCollapsed && (
                            <div className={`flex-1 flex flex-col min-h-0 rounded-lg p-1 ${isArchived ? 'bg-yellow-50/30 dark:bg-yellow-900/10 border border-dashed border-yellow-200 dark:border-yellow-900/30' : 'bg-zinc-50/50 dark:bg-zinc-900/30'}`}>
                                    ...
                                    .map(task => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <h3 className="text-sm font-medium text-zinc-900 dark:text-zinc-100 mb-2 leading-snug">{task.title}</h3>
                                                
                                                <div className="flex flex-wrap gap-1 mb-3">
                                                    {task.placements.map(placement => (
                                                        <span key={placement} className="text-[10px] bg-zinc-50 dark:bg-zinc-800 text-zinc-500 border border-zinc-100 dark:border-zinc-700 px-1 rounded-sm">#{placement}</span>
                                                    ))}
                                                </div>
                                                ...
                                                <div className="flex items-center justify-between pt-2">
                                                    ...
                                                    <div className="flex -space-x-1.5">
                                                        {assignees.length > 0 ? assignees.slice(0, 3).map(a => (
                                                            <img key={a.id} src={a.avatar} alt={a.name} className="w-5 h-5 rounded-full border border-white dark:border-zinc-900 grayscale" title={a.name} />
                                                        )) : (
                                                            <div className="w-5 h-5 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center text-zinc-400"><User size={10} /></div>
                                                        )}
                                                        {assignees.length > 3 && (
                                                            <div className="w-5 h-5 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center text-zinc-400 text-[8px] border border-white dark:border-zinc-900">+{assignees.length - 3}</div>
                                                        )}
                                                    </div>
                                                </div>
                                            </div>
                     ...
                     return (
                         ...
                         >
                             <div className="flex items-center gap-2 sticky top-0 bg-white dark:bg-black z-10 py-2 group/header cursor-grab active:cursor-grabbing">
                                <div className="text-zinc-300 opacity-0 group-hover/header:opacity-100 transition-opacity"><GripVertical size={14}/></div>
                                <button onClick={() => toggleSection(col.id)} className="p-1 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400 hover:text-zinc-600 transition-colors">
                                    {isCollapsed ? <ChevronRight size={16} /> : <ChevronDown size={16} />}
                                </button>
                                <span className="bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 text-xs px-2 py-0.5 rounded font-medium">{colTasks.length}</span>
                                ...
                                {teamFilter !== 'my-work' && (
                                    <div className="relative ml-2">
                                        ...
                                        {activeColumnMenu === col.id && (
                                            <div className="absolute left-0 top-6 w-48 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg shadow-xl z-50 py-1 flex flex-col text-xs" onClick={(e) => e.stopPropagation()}>
                                                <button onClick={() => handleStartRename(col.id, col.label)} className="text-left px-3 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center gap-2">
                                                    <Edit2 size={12} /> Rename
                                                </button>
                                                <button onClick={() => { onDuplicateStatus(teamFilter, col.id, false); setActiveColumnMenu(null); }} className="text-left px-3 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center gap-2">
                             ...
                             {!isCollapsed && (
                                <div className={`overflow-x-auto border border-zinc-200 dark:border-zinc-800 rounded-lg cursor-default ${isArchived ? 'bg-yellow-50/10' : ''}`}>
                                    ...
                                    <table className="w-full text-left text-sm border-collapse min-w-[800px] table-fixed">
                                        <thead className="bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800">
                                            <tr>
                                                ...
                                                {customProperties.map(prop => (
                                                    <th key={prop.id} className="p-3 font-medium text-zinc-400 text-xs w-32 group/prop cursor-pointer relative">
                                                        {editingColumnId === prop.id ? (
                                                            ...
                                                            <div className="flex items-center justify-between" onClick={() => handleStartRename(prop.id, prop.name)}>
                                                                {prop.name}
                                                                <button onClick={(e) => { e.stopPropagation(); setActivePropertyMenu(activePropertyMenu === prop.id ? null : prop.id) }} className="opacity-0 group-hover/prop:opacity-100 hover:text-zinc-900 dark:hover:text-white"><MoreHorizontal size={12}/></button>
                                                            </div>
                                                        )}
                                                        {activePropertyMenu === prop.id && (
                                                            <div className="absolute top-8 right-0 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg shadow-xl z-50 py-1 w-32">
                                                                <button onClick={(e) => { e.stopPropagation(); handleStartRename(prop.id, prop.name); }} className="w-full text-left px-3 py-1.5 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center gap-2 text-xs">
                                                                    <Edit2 size={12} /> Rename
                                                                </button>
                                                                <button onClick={(e) => { e.stopPropagation(); handleDeleteProperty(prop.id); }} className="w-full text-left px-3 py-1.5 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center gap-2 text-xs text-red-500">
                                                                    <Trash2 size={12} /> Delete
                                                                </button>
                                                            </div>
                                                        )}
                                                ...
                                                <th className="p-2 w-10 text-center relative">
                                                    <button onClick={() => setIsAddPropertyOpen(!isAddPropertyOpen)} className="p-1 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded text-zinc-400"><Plus size={14}/></button>
                                                    {isAddPropertyOpen && (
                                                        <div className="absolute right-0 top-8 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg shadow-xl z-50 p-3 w-56 text-left">
                                                            ...
                                                            <div className="space-y-1 mb-3">
                                                                <button onClick={() => setNewPropType('text')} className={`w-full text-left px-2 py-1.5 rounded text-xs flex items-center gap-2 ${newPropType === 'text' ? 'bg-zinc-100 dark:bg-zinc-800 font-medium' : 'hover:bg-zinc-50 dark:hover:bg-zinc-800/50'}`}><Type size={12}/> Text</button>
                                                                <button onClick={() => setNewPropType('select')} className={`w-full text-left px-2 py-1.5 rounded text-xs flex items-center gap-2 ${newPropType === 'select' ? 'bg-zinc-100 dark:bg-zinc-800 font-medium' : 'hover:bg-zinc-50 dark:hover:bg-zinc-800/50'}`}><ListIcon size={12}/> Select</button>
                                                                <button onClick={() => setNewPropType('date')} className={`w-full text-left px-2 py-1.5 rounded text-xs flex items-center gap-2 ${newPropType === 'date' ? 'bg-zinc-100 dark:bg-zinc-800 font-medium' : 'hover:bg-zinc-50 dark:hover:bg-zinc-800/50'}`}><CalendarIcon size={12}/> Date</button>
                                                                <button onClick={() => setNewPropType('person')} className={`w-full text-left px-2 py-1.5 rounded text-xs flex items-center gap-2 ${newPropType === 'person' ? 'bg-zinc-100 dark:bg-zinc-800 font-medium' : 'hover:bg-zinc-50 dark:hover:bg-zinc-800/50'}`}><Users size={12}/> Person</button>
                                                            </div>
                                                            <button onClick={handleCreateProperty} className="w-full bg-black dark:bg-white text-white dark:text-black py-1.5 rounded text-xs font-bold">Create</button>
                                                        </div>
                                                    )}
                                                </th>
                                            </tr>
                                        ...
                                        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800 bg-white dark:bg-zinc-900/40">
                                            {colTasks.length > 0 ? colTasks.map(task => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <td className="p-3 font-medium text-zinc-900 dark:text-zinc-100 border-r border-transparent group-hover:border-zinc-100 dark:group-hover:border-zinc-800 flex items-center gap-2 truncate">
                                                            <GripVertical size={12} className="text-zinc-300 opacity-0 group-hover:opacity-100 flex-shrink-0" />
                                                            <span className="truncate">{task.title}</span>
                                                        </td>
                                                        <td className="p-3 text-zinc-600 dark:text-zinc-400 text-xs truncate">{task.contentInfo?.type || task.type}</td>
                                                        <td className="p-3">
                                                            <div className="flex -space-x-1.5">
                                                                {authors.map(a => (
                                                                        <img key={a.id} src={a.avatar} className="w-5 h-5 rounded-full grayscale border border-white dark:border-zinc-900 flex-shrink-0" title={a.name} />
                                                                ))}
                                                                {authors.length === 0 && '-'}
                                                            </div>
                                                        </td>
                                                        <td className={`p-3 text-xs ${PRIORITY_COLORS[task.priority]} capitalize truncate`}>{task.priority}</td>
                                                        <td className="p-3 text-zinc-500 text-xs truncate">{new Date(task.dueDate).toLocaleDateString('en-US')}</td>
                                                        
                                                        {/* Custom Properties Render */}
                                                        {customProperties.map(prop => (
                                                            <td key={prop.id} className="p-3 text-xs text-zinc-600 dark:text-zinc-400 truncate">
                                                                {task.customFieldValues?.[prop.id] ? String(task.customFieldValues[prop.id]) : '-'}
                                                            </td>
                                                        ))}

                                                        <td className="p-3 text-zinc-500 text-xs">
                                                            <div className="flex gap-1 overflow-hidden">
                                                                {task.placements.map(tag => <span key={tag} className="truncate">#{tag}</span>)}
                                                            </div>
                                                        </td>
                                                        <td className="p-3"></td>
                                                    </tr>
                                                ...
                                                <tr>
                                                    <td colSpan={6 + customProperties.length} className="p-4 text-center text-xs text-zinc-400 italic">No tasks in this step</td>
                                                </tr>
                                            )}
                                            {/* Add Task Row */}
                                            {!searchQuery && (
                                                <tr className="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors cursor-pointer" onClick={() => onAddTask({ status: col.id })}>
                                                    <td colSpan={6 + customProperties.length} className="p-2 pl-3 text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 text-xs font-medium flex items-center gap-2">
            ...
            <div className="h-full flex flex-col bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg overflow-hidden relative">
                ...
                <div className="flex items-center justify-between p-3 bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 flex-shrink-0">
                    <div className="flex items-center gap-2">
                        <button onClick={() => changeMonth(-1)} className="p-1 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded text-zinc-600"><ChevronLeft size={16}/></button>
                        <h3 className="text-sm font-bold text-zinc-900 dark:text-white w-32 text-center">
                            {monthNames[calendarDate.getMonth()]} {calendarDate.getFullYear()}
                        </h3>
                        <button onClick={() => changeMonth(1)} className="p-1 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded text-zinc-600"><ChevronRight size={16}/></button>
                    </div>
                    <div></div> 
                </div>
                ...
                <div className="grid grid-cols-7 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 flex-shrink-0">
                    {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
                        <div key={d} className="p-2 text-center text-[10px] font-bold text-zinc-400 uppercase tracking-wider border-r border-zinc-200 dark:border-zinc-800 last:border-r-0">{d}</div>
                    ))}
                </div>
                     ...
                     <div className="grid grid-cols-7 auto-rows-fr border-b border-zinc-200 dark:border-zinc-800 min-h-[500px]">
                        {calendarDays.map((dateObj, i) => {
                            ...
                            if (!isCurrentMonth) {
                                ...
                                return <div key={i} className="bg-zinc-50/20 dark:bg-zinc-900/20 border-b border-r border-zinc-200 dark:border-zinc-800 last:border-r-0"></div>;
                            }

                            const dayTasks = filteredTasks.filter(t => new Date(t.dueDate).toDateString() === dateObj.date.toDateString());
                            ...
                            return (
                                ...
                                >
                                    <div className={`p-1 text-[10px] font-medium text-right ${isToday ? 'text-red-500 font-bold' : 'text-zinc-400'} group-hover/cell:text-zinc-900 dark:group-hover/cell:text-zinc-100`}>
                                        {dateObj.day}
                                    </div>
                                    <div className="flex-1 p-1 space-y-1">
                                        {dayTasks.map(t => (
                                            ...
                                            >
                                                <div className={`text-[9px] px-1.5 py-1 rounded border shadow-sm flex flex-col gap-0.5 ${STATUS_COLORS[t.status] || 'bg-white border-zinc-200'}`}>
